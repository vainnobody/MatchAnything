...
def roma_outdoor(device, weights=None, dinov2_weights=None, coarse_res = 560, upsample_res = 864, upsample_preds = True, symmetric=True, attenuate_cert=True):
    ...
    if dinov2_weights is None:
        dinov2_weights = torch.hub.load_state_dict_from_url(weight_urls["dinov2"],
                                                     map_location=device)
    model = roma_model(resolution=coarse_res, upsample_preds=upsample_preds,
               weights=weights,dinov2_weights = dinov2_weights,device=device, symmetric=symmetric, attenuate_cert=attenuate_cert)
    model.upsample_res = upsample_res
    print(f"Using coarse resolution {coarse_res}, and upsample res {model.upsample_res}")
    return model

def roma_indoor(device, weights=None, dinov2_weights=None, coarse_res = 560, upsample_res = 864):
    if isinstance(coarse_res, int):
        coarse_res = (coarse_res, coarse_res)
    if isinstance(upsample_res, int):    
        upsample_res = (upsample_res, upsample_res)
